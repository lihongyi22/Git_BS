...
function renderExperimentTrial() {
  ...
  const curveLabel = t.glucose_trend_label ? ` ? ${t.glucose_trend_label}` : "";
  const chartModeLabel = activeAction ? `${actionLabel(activeAction)} · 行为结果反馈${curveLabel}` : `基础血糖曲线${curveLabel}`;
    ...
    : (finalResultVisible
      ...
      : (state.step === 2
        ? (state.glucoseStreamingActive
          ? "血糖曲线逐点展示中，建议会在 +60min 停止更新后自动播放。"
          : "已展示到 +60min，可继续选择决策。")
        : (state.glucoseStreamingActive
          ? "血糖曲线逐点展示中，基础预测会在 +60min 后出现。"
          : "基础血糖曲线正在逐点展示。")));
              ...
              <div class="glucose-predictions">
                ...
                ` : `
                  <div class="pred-item" style="grid-column: 1 / -1;">
                    ...
                    <div class="glucose-summary-text">曲线展示到 +60min 后显示基础预测</div>
                  </div>
                `}
              </div>
            </div>
        ...
        <div class="panel advice-panel ${state.step === 2 ? 'visible' : ''}">
          ${state.step === 2 ? `
            ...
            <div class="audio-player">
              ${state.glucoseStreamingActive ? `<div class="advice-status">建议将在曲线更新到 +60min 后自动播放</div>` : ''}
...
function renderGlucoseChart(series, axis, modeLabel, summary) {
    ...
    ? `
      <div class="glucose-summary-grid">
        <div class="glucose-summary-card">
          <div class="pred-label">+30min</div>
        ...
        <div class="glucose-summary-card">
          <div class="pred-label">+60min</div>
    ...
    : `
      <div class="glucose-summary-grid">
        <div class="glucose-summary-card accent-card" style="grid-column: 1 / -1;">
          <div class="pred-label">预测值</div>
          <div class="glucose-summary-text strong">曲线展示到 +60min 后显示基础预测</div>
        </div>
      </div>
    `;
  ...
  const displayedSeries = state.glucoseStreamingActive 
    ...
    : ((state.step1ResultVisible || state.decisionSubmitted) 
      ...
      : series.filter(point => point.minute <= 60));  // 未提交时只显示到+60min预测
  ...
  const visiblePointsInChart = displayedSeries.map((point, idx) => {
    ...
    // 线条颜色
    const lineColor = point.kind === 'decision' 
      ? 'var(--warn)' 
      : point.kind.startsWith('outcome') 
      ? 'var(--success)' 
      : 'var(--accent)';
    ...
    return {
      ...
      labels: `
        <text x="${x}" y="${y - 14}" class="glucose-point-label" text-anchor="middle">${point.value.toFixed(1)}</text>
        <text x="${x}" y="${y + 23}" class="glucose-point-time" text-anchor="middle">${glucoseMinuteLabel(point.minute)}m</text>
    ...
    <div class="glucose-chart-shell">
      <div class="glucose-chart-header">
        <div>
          ...
          <div class="glucose-chart-subtitle">15min 时间轴 · 动态反馈 (${displayedSeries.length > 0 ? displayedSeries[displayedSeries.length - 1].minute : 0}min)</div>
          <div class="glucose-axis-title">单位：${axis?.unit || "mmol/L"} · 时间轴从 -30min 到 +150min | 提示：图表支持拖动操作</div>
        </div>
        <div class="glucose-legend">
          <span><i class="legend-swatch history"></i>历史</span>
          <span><i class="legend-swatch meal"></i>进食</span>
          <span><i class="legend-swatch decision"></i>决策</span>
          <span><i class="legend-swatch future"></i>后续</span>
        </div>
      </div>
