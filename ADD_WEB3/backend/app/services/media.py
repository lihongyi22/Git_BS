...
VOICE_MAP = {
    "P": "voice_iii_expert.wav",
    "Q": "voice_iv_expert.wav",
    "R": "voice_iii_peer.wav",
    "S": "voice_iv_peer.wav",
...
VIDEO_MAP = {
    "P": "video_iii_expert.mp4",
    "Q": "video_iv_expert.mp4",
    "R": "video_iii_peer.mp4",
    "S": "video_iv_peer.mp4",
...
def resolve_media_urls(condition_id: str, media: str) -> tuple[str | None, str | None]:
    ...
    if media in {"audio", "voice"}:
        filename = find_media_file(voice_path, condition_id, "audio")
        if not filename:
            filename = next((p.name for p in sorted(voice_path.glob("*.wav"))), None) or next((p.name for p in sorted(voice_path.glob("*.mp3"))), None)
        if filename:
            return (f"/media/voice/{filename}?t={int(Path(voice_path / filename).stat().st_mtime)}", None)
        return (None, None)
    
    if media in {"avatar", "digital_human"}:
        filename = find_media_file(video_path, condition_id, "video")
        if not filename:
            filename = next((p.name for p in sorted(video_path.glob("*.mp4"))), None)
        if filename:
